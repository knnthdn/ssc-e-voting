...
const electionSeed: ElectionSeed = {
  name: "Sample Student Council Election (Existing Voters)",
  description:
    "Sample election seeded with complete positions and candidates where existing voters cast votes.",
...
function buildCandidateName(positionIndex: number, partyIndex: number): {
  firstName: string;
  lastName: string;
  gender: Gender;
} {
...
async function getExistingVoters(partyStrengths: number[]): Promise<ExistingVoterChoice[]> {
  ...
  if (voters.length === 0) {
    throw new Error(
      "No existing voters found. Create voters first before running this seed.",
    );
  }
...
async function main() {
  ...
  const voters = await getExistingVoters(partyStrengths);
  ...
  console.log(`Existing voters used: ${voters.length}`);
  console.log(`Positions seeded: ${POSITION_NAMES.length}`);
  console.log(`Candidates per position: ${CANDIDATES_PER_POSITION}`);
  console.log(`Votes per position: ${voters.length}`);
}

main()
  .catch((error) => {
    console.error("Failed to seed election data using existing voters:", error);
    process.exitCode = 1;
  })
  .finally(async () => {
